...
export STAGING_ID=$(curl -s -u $SONATYPE_USERNAME:$SONATYPE_PASSWORD \
    ...
    "$API_ENDPOINT/staging/profiles/$MR_CLIENT_PROFILE_ID/start" \
...
export CLOSE_PAYLOAD="<promoteRequest>\
    <data>\
        ...
        <description>Close MapRoulette Java Client repo</description>\
    </data>\
</promoteRequest>"
...
curl --fail -s -u $SONATYPE_USERNAME:$SONATYPE_PASSWORD \
    ...
    -H "Content-Type:application/xml" \
    -d "$CLOSE_PAYLOAD" \
    "$API_ENDPOINT/staging/profiles/$MR_CLIENT_PROFILE_ID/finish"
