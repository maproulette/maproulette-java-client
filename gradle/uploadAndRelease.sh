...
export MR_JAVA_CLIENT_PROFILE_ID=1442a4f451744
export DESCRIPTION_PAYLOAD="<promoteRequest>\
    <data>\
        <description>MR API Client Release</description>\
    </data>\
</promoteRequest>"
...
export STAGING_ID=$(curl -s -u $SONATYPE_USERNAME:$SONATYPE_PASSWORD \
    ...
    "$API_ENDPOINT/staging/profiles/$MR_JAVA_CLIENT_PROFILE_ID/start" \
    | perl -nle 'print "$1" if ($_ =~ /.*<stagedRepositoryId>(.*)<\/stagedRepositoryId>.*/g);' \
    | awk '{$1=$1};1')
# Response parsed looks like this:
# <promoteResponse>  <data>    <stagedRepositoryId>orgopenstreetmapclient-1147</stagedRepositoryId>    <description>MR Java Client Release</description>  </data></promoteResponse>

export CLOSE_PAYLOAD="<promoteRequest>\
    <data>\
        <stagedRepositoryId>$STAGING_ID</stagedRepositoryId>\
        <description>Close MR API Client repo</description>\
    </data>\
</promoteRequest>"
...
curl --fail -s -u $SONATYPE_USERNAME:$SONATYPE_PASSWORD \
    ...
    -H "Content-Type:application/xml" \
    -d "$CLOSE_PAYLOAD" \
    "$API_ENDPOINT/staging/profiles/$MR_JAVA_CLIENT_PROFILE_ID/finish"
